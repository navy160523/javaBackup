...
public class Ex2 {
	...
	public static void main(String[] args) {
		...
		SmartPhone sp = new SmartPhone("갤럭시z플립3","010-1234-5678","안드로이드");
...
//			hp.kakaoTalk(); // SmartPhone 의 기능은 사용을 포기해야 한다!
			...
			System.out.println("sp 는 HdandPhone 이 아니다!");
		}
...
//		sp = (SmartPhone)hp;
		...
		if(hp instanceof SmartPhone) {
			...
			System.out.println("hp 는 SmartPhone 으로 변환 불가능!");
		...
		HandPhone hp2 = new SmartPhone("갤럭시z플립3","010-1234-5678","안드로이드");
...
//			SmartPhone sp2 = hp2; // 자동 형변환 불가능!
		...
		// Child(c) -> Parent(p) 형변환 가능 여부 체크
		// 형변환을 바로 수행하지 않고, instanceof 연산자를 통해서 검사 후 형변환 수행
...
//		Parent p2 = new Child(); // 자동형 변환
		
		// Parent 타입(p2) -> Child(c2) 형변환 가능 여부 체크
		// 형변환을 바로 수행하지 않고, instanceof 연산자를 통해서 검사 후 형변환 수행
...
class SmartPhone extends HandPhone {
	...
	// => 모델명과 전화번호는 슈퍼클래스의 생성자를 통해 대신 초기화를 수행

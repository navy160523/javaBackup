...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * - 슈퍼클래스타입 <-> 서브클래스타입
		 * - 묵시적 형변환(업캐스팅, Up Casting, 자동타입변환) 과
...
//		p.parentPrn();
		...
		// ---------- 업캐스팅(서브클래스 타입 -> 슈퍼클래스 타입) ---------------
		byte bNum = 10;
		int iNum = bNum; // 자동형변환(작은타입 -> 큰타입)
		
		// 슈퍼클래스 타입 Parent 의 레퍼런스 변수 선언
		Parent p;
		// 슈퍼클래스 타입 변수 p에 서브클래스 타입 Child 인스턴스 주소를 전달
		p = c; // 묵시적 형변환(업캐스팅)
...
//		p.childPrn(); // 서브클래스에서 정의한 메서드(접근 불가) => 오류발생
...
//		c2.childPrn(); // 실제 Parent 인스턴스에 존재하지 않는 메서드
		...
		Parent p3 = new Child(); // 자동(묵시적) 형변환이 일어남
...
//		Child c3 = p3; // 컴파일 에러 발생! 명시적 형변환 필수!
		 ...
		 *   = 슈퍼클래스의 인스턴스를 서브클래스 타입으로 형변환하는 것
		 * - 참조 가능한 영역이 확대됨
		 * - 자동 형변환은 일어나지 않음 = 명시적(강제적) 형변환 필요
		 * - 강제 형변환을 통해 구문 오류가 해결되더라도
		 *   실행 시점에서 오류(실행 오류)가 발생하게 됨
		 *   => 존재하지 않는 영역의 참조 위험성 때문
		 * ============================================================

...
public class Ex5 {
	...
	public static void main(String[] args) {
		 ...
		 * - sleep() 메서드 파라미터로 밀리초 단위 또는 나노초 단위의 시간을
		 *   설정하면 해당 시간이 만료되면 다시 실행대기 상태로 변경됨
		 *   => 단, 타이머가 동작 중 interrupt() 메서드가 호출되면
		 ...
		 * 		// interrupt() 메서드가 호출 될 때(쓰레드가 깨어날 때)
		 * 		// 실행할 작업을 기술...
		 * }
		 * 
		 *   
		...
		t1.setPriority(Thread.MIN_PRIORITY); // 우선 순위 1로 변경
		t2.setPriority(Thread.MAX_PRIORITY); // 우선 순위 10으로 변경
...
//		t2.start();
		...
		for(int i = 1; i <= 10; i++) {
			...
			try {
				Thread.sleep(1000);
				// 1초 단위로 시각이 출력되도록 1초씩 재움
				// => 정확히 1초는 아니고 1초보다 약간 더 걸림
			} catch (InterruptedException e) {
